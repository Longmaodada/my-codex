...
export function SettingsPage({ settings, onChange, onResetData }: { settings: AppSettings; onChange: (patch: Partial<AppSettings>) => void; onResetData: () => Promise<void> }) {
  ...
  return (
    <div className="settings-layout">
      ...
      <GlassCard className="settings-panel">
        ...
        {section === "数据" && <div className="settings-group data-settings-group"><Toggle label="Mock Mode" description="使用演示数据验证界面，不写入真实统计库" checked={settings.mockMode} onChange={(value) => onChange({ mockMode: value })} /><DataResetCard onReset={onResetData} /></div>}
        ...
        {section === "关于" && <div className="privacy-card"><Info weight="fill" /><h3>My Codex {__APP_VERSION__}</h3><p>本项目是独立的本地分析工具，与 OpenAI 没有官方隶属关系。</p></div>}
      </GlassCard>
    </div>
  );
}
